...
- CI uses SHA-pinned actions with `# vX.Y.Z` comments and digest-pinned Docker base images. Dependabot maintains both, but NOT `.mise.toml` (it can't parse it): bump `rust`/`cargo-llvm-cov`/`actionlint` by hand, `mise outdated` lists them. New actions/base images must follow the same form.
